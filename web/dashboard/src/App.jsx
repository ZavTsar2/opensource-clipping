...
function ProgressCard({ job, onRefresh }) {
  const progress = job.progress || {}; const percent = Math.max(4, Math.round(progress.percent || 0))
  return <section className="progress-card"><div><p className="eyebrow">CURRENT JOB</p><h2>{stageName(job.status)}</h2><p>{progress.message || 'Your notebook has queued this clip job.'}</p></div><button className="text-button" onClick={onRefresh}>Refresh</button><div className="progress"><i style={{ width: `${percent}%` }} /></div><small>{percent}% · {job.config?.clips || 5} clips requested</small></section>
}
...
function Jobs({ jobs, onRefresh }) {
  ...
  return <section className="history"><div className="section-heading"><div><p className="eyebrow">LOCAL HISTORY</p><h2>{completed.length ? 'Ready to download' : 'Recent jobs'}</h2></div><span>Session links can expire</span></div>{jobs.map((job) => <JobCard job={job} key={job.id} onRefresh={onRefresh} />)}</section>
}

function JobCard({ job, onRefresh }) {
  const [media, setMedia] = useState({}); const [mediaError, setMediaError] = useState('')
  async function loadClip(clip) {
    if (media[clip.filename]) return
    try {
      const mediaUrl = await fetchMedia(clip.download_url)
      setMedia((current) => ({ ...current, [clip.filename]: mediaUrl }))
    } catch (err) { setMediaError(err.message) }
  }
  return <article className="job-card"><div className="job-top"><div><span className={`status ${job.status}`}>{job.status || 'saved'}</span><h3>{job.url || 'Previous clip job'}</h3></div><button className="text-button" onClick={() => onRefresh(job.id)}>Refresh</button></div>{job.error && <p className="job-error">{job.error}</p>}{job.status === 'completed' && <div className="clip-grid">{(job.clips || []).map((clip) => <div className="clip" key={clip.filename}><div className="clip-preview">{media[clip.filename] ? <video src={media[clip.filename]} controls preload="metadata" /> : <button onClick={() => loadClip(clip)}>Load preview</button>}</div><div><b>#{clip.rank} {clip.title || clip.title_en || 'Clip'}</b><span>{clip.duration ? `${Math.round(clip.duration)} sec` : 'Ready'}{typeof clip.start_time === 'number' && typeof clip.end_time === 'number' ? ` · ${formatTimestamp(clip.start_time)}–${formatTimestamp(clip.end_time)}` : ''}{clip.viral_score ? ` · Score ${clip.viral_score}` : ''}</span>{media[clip.filename] ? <a href={media[clip.filename]} download={clip.filename}>Download MP4</a> : <button className="text-button" onClick={() => loadClip(clip)}>Prepare download</button>}</div></div>)}</div>}{mediaError && <p className="job-error">{mediaError}</p>}</article>
}
